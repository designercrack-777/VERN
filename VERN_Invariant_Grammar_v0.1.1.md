# VERN Invariant Grammar Specification v0.1.1
...
```
PROGRAM ::= (FILE_VALUE | CONTAINER | SCRIPT | COMMENT)* [ENTRY_POINT] [STOP]

FILE_VALUE ::= SET REFERENCE TO EXPRESSION
...
CONVERT_INSTRUCTION ::= CONVERT_KW (REFERENCE | LOOP_KW) TO_KW TYPE_KW AS_KW REFERENCE
...
| `IF_KW` | conditional |
| `THEN_KW` | conditional action marker |
...
| `AS_KW` | definition assignment marker |
...
Compound comparison operators (`IS_EQUAL_TO_KW`, `IS_GREATER_THAN_KW`, etc.) may be mapped to multi-word phrases. The parser treats each compound as a single atomic token.
...
```
// Multilingual greeter — English binding
...
hati .hesabu
    ...
    ikiwa .opereta = "-" basi weka .namba3 kwa .namba1 - .namba2
    ikiwa .opereta = "x" au .opereta = "*" basi weka .namba3 kwa .namba1 * .namba2
    ikiwa .opereta = "/" basi weka .namba3 kwa .namba1 / .namba2
...
**Japanese — Calculator Example:**
```
// 電卓 — Japanese binding

スクリプト .計算
    入力する .入力1
    変換する .入力1 に 数字 として .数字1
    表示する .数字1
    入力する .演算子
    表示する .数字1, .演算子
    入力する .入力2
    変換する .入力2 に 数字 として .数字2
    表示する .数字1, .演算子, .数字2
    もし .演算子 = "+" ならば 設定する .数字3 に .数字1 + .数字2
    もし .演算子 = "-" ならば 設定する .数字3 に .数字1 - .数字2
    もし .演算子 = "x" または .演算子 = "*" ならば 設定する .数字3 に .数字1 * .数字2
    もし .演算子 = "/" ならば 設定する .数字3 に .数字1 / .数字2
...
**Arabic — Calculator Example:**
```
// آلة حاسبة — Arabic binding

نص .حساب
    اسأل .إدخال١
    حوّل .إدخال١ إلى رقم بوصفه .رقم١
    اعرض .رقم١
    اسأل .عملية
    اعرض .رقم١, .عملية
    اسأل .إدخال٢
    حوّل .إدخال٢ إلى رقم بوصفه .رقم٢
    اعرض .رقم١, .عملية, .رقم٢
    إذا .عملية = "+" فإن اضبط .رقم٣ إلى .رقم١ + .رقم٢
    إذا .عملية = "-" فإن اضبط .رقم٣ إلى .رقم١ - .رقم٢
    إذا .عملية = "x" أو .عملية = "*" فإن اضبط .رقم٣ إلى .رقم١ * .رقم٢
    إذا .عملية = "/" فإن اضبط .رقم٣ إلى .رقم١ / .رقم٢
    اعرض .رقم١, .عملية, .رقم٢, "=", .رقم٣
نهاية نص
...
2. **No collisions** — no two tokens may share the same mapped keyword. If a target language uses the same word for two different concepts, one must be disambiguated (by adding a modifier, choosing a synonym, or using a phrase)

3. **Imperative mood** — keywords should use imperative verb forms where the target language has them. VERN is a command language; the vocabulary should reflect this

4. **Unicode compliance** — all keywords must be valid Unicode. No characters outside the Unicode standard are permitted

5. **Symbol preservation** — the symbol forms (`+`, `-`, `*`, `/`, `=`, `!=`, `>`, `<`, `>=`, `<=`, `.`, `#`, `"`, `,`, `//`) are identical in all bindings. They are not localized
...
7. **Cultural mapping via `define`** — bindings are encouraged to include a standard library of `define` extensions mapping culturally native concepts to VERN operations. These are not required for binding validity but significantly improve usability
