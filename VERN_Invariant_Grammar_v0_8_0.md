# VERN Invariant Grammar Specification v0.8.0
...
END_CONTAINER ::= END_KW HASH IDENTIFIER
...
// Note: HASH (the `#` structural symbol, used for container tags) is a distinct
// token from HASH_KW (the `hash` keyword, used for Argon2id hashing). They are
// unrelated despite the similar name — do not conflate them in implementation.
...
| `CURRENT_FOLDER_KW` | program file's own directory (two-word compound: current folder) |
...
| `SECONDS_KW` | seconds unit |
| `MILLISECONDS_KW` | milliseconds unit |
| `HASH_KW` | one-way hash of a text value (Argon2id) |
| `VERIFY_KW` | verify a text value against a hash, opening keyword |
| `MATCH_KW` | comparison marker inside a `verify` instruction |
| `CREATE_KW` | opening keyword of a create-id instruction |
| `ID_KW` | unique identifier marker, second word of `create id` |
| `MAIL_KW` | send an email via SMTP — opening keyword, distinct from `SEND_KW` (HTTP POST) |
| `SUBJECT_KW` | email subject line marker |
| `BODY_KW` | email body marker |
| `ATTACH_KW` | email attachment marker — followed by `none` or one or more file paths |
...
FOLDER_KW                          folder
PARENT_KW                          parent
CURRENT_FOLDER_KW                  current folder
COSINE_KW                          cosine
TANGENT_KW                         tangent
ARCSINE_KW                         arcsine
...
CURRENT_FOLDER_KW                  folda ya sasa
COSINE_KW                          kosinusi
TANGENT_KW                         tanjenti
...
PARENT_KW                          ルート                 rūto
CURRENT_FOLDER_KW                  現在のフォルダ         genzai no foruda
...
HASH_KW                            ハッシュ化する         hasshu-ka suru
VERIFY_KW                          検証する               kenshō suru
MATCH_KW                           一致確認               icchi kakunin
CREATE_KW                          作成する               sakusei suru
ID_KW                              識別子                 shikibetsu-shi
MAIL_KW                            メール送信する         mēru sōshin suru
SUBJECT_KW                         件名                   kenmei
BODY_KW                            本文                   honbun
ATTACH_KW                          添付する               tenpu suru
```
...
CURRENT_FOLDER_KW                  المجلد الحالي          al-mijlad al-ḥālī
...
HASH_KW                            جزّئ                   jazziʾ
VERIFY_KW                          تحقّق                  taḥaqqaq
MATCH_KW                           تطابق                  taṭābuq
CREATE_KW                          أنشئ                   anshiʾ
ID_KW                              معرّف                  muʿarrif
MAIL_KW                            أرسل بريدًا            arsil barīdan
SUBJECT_KW                         موضوع                  mawḍūʿ
BODY_KW                            نص الرسالة             naṣṣ al-risāla
ATTACH_KW                          أرفق                   arfiq
```
