# VERN Language Specification v0.6
...
This document is the full specification for v0.6, covering syntax, grammar rules, reference system, data handling, collections, dictionaries, nested data structures, imports, extended math, trigonometry, logarithms, mathematical constants, angle conversion, conditionals, repetition, loop control, scripts, script parameters, error recovery, date and time, string operations, file operations, core operations, type checking, number formatting, multi-line text, and networking.
...
VERN supports two HTTP operations: fetching data from a URL and sending data to a URL. These are the only networking operations in v0.6. All other network concerns — authentication, headers, response codes — are deferred to future versions or library implementations.
...
Sends an HTTP GET request to the specified URL and assigns the response body to a new named value as text. The URL must be a text value. The result is always text — the raw response body exactly as the server returns it. Parsing the response is the program's responsibility.
...
Sends an HTTP POST request to the specified URL with the specified data as the request body. Both `.data` and `.url` must be text values. No response is captured — if the response body is needed, that is a future concern.

```
set .url to "https://api.example.com/submit"
set .payload to "name=gabriel&score=100"
send .payload to .url
```

### Error Recovery

Network operations can fail for reasons outside the program's control — no connection, server unavailable, invalid URL, timeout. Network failures are fatal by default but fully recoverable inside an `attempt` block, consistent with file operations and type conversions.
...
`fail reason` inside the fail handler describes what went wrong — connection refused, timeout, invalid URL, and so on.
...
### Networking Error Behaviors

**`fetch` with a non-text URL value** — fatal. Error log records the value and its type.

**`send` with non-text data or URL** — fatal. Error log records the value and its type.

**Network request fails at runtime** — fatal by default. Recoverable inside an `attempt` block. `fail reason` describes the failure.

**Invalid URL format** — fatal at runtime when the request is attempted. Error log records the URL value.

**`fetch` or `send` with an uninitialized value** — fatal. Error log records the value name and line number.

**`fetch` returns an empty response body** — the result is assigned an empty string. Not an error. Consistent with empty file reads and empty user input.

### Known Limitations

Response headers, status codes, request headers, authentication, and non-POST methods are not supported in v0.6. These are library concerns. The core language provides only the fundamental send and receive operations.
...
Deeply nested structures — a dictionary of dictionaries of dictionaries, for example — are not supported in v0.6. Two levels only: an outer structure containing inner dictionaries. This covers the vast majority of real-world use cases. Deeper nesting is deferred to a future version.
...
**Deeply nested structures** — nested data structures are limited to two levels in v0.6. A dictionary of dictionaries of dictionaries is not supported. Deferred to a future version.

**Locale-specific number separators** — the decimal separator is `.` and the thousands separator is `,`. These are fixed in v0.6. Locale-aware separator characters will be addressed by the container system in a future version.

**Networking — response headers and status codes** — `fetch` returns only the response body as text. Headers, status codes, request headers, authentication, and non-POST methods are not supported. Deferred to library implementations.
